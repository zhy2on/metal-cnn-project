...
void cnn_seq(float* images, float* network, int* labels, float* confidences,
			 int num_of_image) {
	float* w[21];
	float* b[21];
	int offset = 0;
	...
	for (int i = 0; i < 17; ++i) {
		if (i == 2 || i == 5 || i == 9 || i == 13)
			i++;  // pooling layer has no weights and biases
		w[i] = network + offset;
	...
	for (int i = 0; i < num_of_image; ++i) {
		convolution(images, layer[0], w[0], b[0], INPUT_DIM[0], OUTPUT_DIM[0],
					NBYN[0]);
		convolution(layer[0], layer[1], w[1], b[1], INPUT_DIM[1], OUTPUT_DIM[1],
					NBYN[1]);
