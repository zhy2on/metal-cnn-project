...
static void convolution_metal(id<MTLBuffer> input, id<MTLBuffer> output, id<MTLBuffer> network,
    ...
    @autoreleasepool {
        ...
        // nbyn이 TILE_SIZE의 배수가 아닐 경우를 위한 처리
        int gridWidth = (nbyn + TILE_SIZE - 1) / TILE_SIZE * TILE_SIZE;
        int gridHeight = (nbyn + TILE_SIZE - 1) / TILE_SIZE * TILE_SIZE;
        MTLSize gridSize = MTLSizeMake(gridWidth, gridHeight, outDim * BATCH_SIZE);
